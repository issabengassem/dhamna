...
export default function Home(){
  ...
  useEffect(()=>{
    ...
    async function refreshPublished(){
      ...
      pending=true;
      try{
        const response=await fetch('/api/cms/home',{cache:'no-store',signal:controller.signal});
        const data=response.ok?await response.json():null;
        if(active&&typeof data?.heroEyebrow==='string')setHeroEyebrow(data.heroEyebrow);
      }catch{}finally{pending=false}
    }
    void refreshPublished();
    const interval=window.setInterval(refreshPublished,5000);
    window.addEventListener('focus',refreshPublished);
    document.addEventListener('visibilitychange',refreshPublished);
    return()=>{active=false;controller.abort();window.clearInterval(interval);window.removeEventListener('focus',refreshPublished);document.removeEventListener('visibilitychange',refreshPublished)};
  },[]);
  useEffect(()=>{
